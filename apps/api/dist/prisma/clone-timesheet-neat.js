...
const TARGET_PROJECTS_BY_WORKSPACE = {
    'Captive +Simple': [
...
async function main() {
    ...
    const sourceProject = sourceProjects.find((project) => project.name === 'Création captive Malt - NEAT') ??
    ...
    const sourceByDescription = new Map(sourceTasks.map((task) => [task.description, task.id]));
    ...
    for (const targetWorkspace of targetWorkspaces) {
        ...
        const targetByDescription = new Map(targetTasks.map((task) => [task.description, task.id]));
        const targetByOrder = new Map(targetTasks.map((task) => [task.orderNumber, task.id]));
        const taskIdMapping = new Map();
        for (const sourceTask of sourceTasks) {
            const mappedTargetTaskId = targetByDescription.get(sourceTask.description) ?? targetByOrder.get(sourceTask.orderNumber);
        ...
        const seen = new Set(existingEntries.map((entry) => `${entry.userId}|${entry.taskId ?? ''}|${entry.entryDate.toISOString()}|${entry.minutesSpent}`));
        const toCreate = [];
...
//# sourceMappingURL=clone-timesheet-neat.js.map