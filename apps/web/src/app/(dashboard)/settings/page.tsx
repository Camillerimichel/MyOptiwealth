...
type WorkspaceMembership = { workspace: { id: string; name: string }; role: string };
type UserRole = { user: { id: string; email: string }; role: string };
...
export default function SettingsPage() {
  ...
  const load = useCallback(async (): Promise<void> => {
    ...
    try {
      setLoading(true);
      setError(null);
      const [workspacesData, usersData, auditData] = await Promise.all([
        apiClient.listWorkspaces(token),
        apiClient.listUsers(token),
        apiClient.listAudit(token),
      ]);
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <article className="rounded-xl border border-[var(--line)] bg-white p-5 shadow-panel">
          ...
          <ul className="mt-3 grid gap-2 text-sm">
            {workspaces.map((item) => (
              <li key={item.workspace.id} className="flex items-center justify-between gap-2">
                <span>{item.workspace.name} ({item.role})</span>
                <button onClick={() => onSwitch(item.workspace.id)} className="rounded border border-[var(--line)] px-2 py-1 text-xs">Switch</button>
              </li>
            ))}
          </ul>
        </article>
        ...
        <article className="rounded-xl border border-[var(--line)] bg-white p-5 shadow-panel">
          ...
          <ul className="mt-3 grid gap-2 text-sm">
            {users.map((item) => <li key={item.user.id}>{item.user.email} ({item.role})</li>)}
          </ul>
        </article>
        ...
        <article className="rounded-xl border border-[var(--line)] bg-white p-5 shadow-panel">
          ...
          <ul className="mt-3 grid gap-2 text-sm">
            {audit.slice(0, 10).map((item) => <li key={item.id}>{item.action} | {new Date(item.createdAt).toLocaleString()}</li>)}
          </ul>
        </article>
      </div>
      <article className="rounded-xl border border-[var(--line)] bg-white p-5 shadow-panel">
        ...
        <button onClick={onSaveSettings} className="mt-3 rounded bg-[var(--brand)] px-3 py-2 text-white">Enregistrer settings</button>
      </article>
    </section>
  );
}
