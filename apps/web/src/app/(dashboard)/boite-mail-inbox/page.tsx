...
type AssignmentLevel = 'workspace' | 'project' | 'task';
type LinkSelection = { workspaceId: string; projectId: string; taskId: string; level: AssignmentLevel };
type EmailContent = {
  text: string;
...
export default function MailboxPage() {
    ...
    try {
      ...
      setLinkSelectionByEmail((prev) => {
        ...
        for (const email of emailsData) {
          next[email.id] = prev[email.id] ?? { workspaceId: '', projectId: '', taskId: '', level: 'workspace' };
        }
        return next;
      });
    } catch {
  ...
  function onProjectSelection(emailId: string, projectId: string): void {
    setLinkSelectionByEmail((prev) => {
      const current = prev[emailId] ?? { workspaceId: '', projectId: '', taskId: '', level: 'workspace' };
      return {
        ...prev,
        [emailId]: {
    ...
    setLinkSelectionByEmail((prev) => {
      ...
      return {
        ...
        [emailId]: {
          ...current,
          taskId,
          level: taskId ? 'task' : (current.projectId ? 'project' : 'workspace'),
        },
      };
    });
  }

  function onAssignmentLevelSelection(emailId: string, level: AssignmentLevel): void {
    setLinkSelectionByEmail((prev) => {
      const current = prev[emailId] ?? { workspaceId: '', projectId: '', taskId: '', level: 'workspace' };
      if (level === 'workspace') {
        return {
  ...
  function openAssignModal(email: EmailMessage): void {
    const existing = linkSelectionByEmail[email.id] ?? { workspaceId: '', projectId: '', taskId: '', level: 'workspace' };
    setLinkSelectionByEmail((prev) => ({
      ...prev,
      [email.id]: existing,
    }));
  ...
  async function onLinkEmailFromModal(): Promise<void> {
    ...
    if (!selection?.workspaceId) {
      setAssignStatus('error');
      setAssignMessage('Sélectionne un workspace.');
      return;
    }
    if (selection.level === 'project' && !selection.projectId) {
      setAssignStatus('error');
      setAssignMessage('Sélectionne un projet.');
      return;
    }
    if (selection.level === 'task' && (!selection.projectId || !selection.taskId)) {
      setAssignStatus('error');
      setAssignMessage('Sélectionne un projet et une tâche.');
      return;
    }
    ...
    try {
      await apiClient.linkInboxEmail(token, {
        ...
        projectId: selection.level === 'workspace' ? undefined : selection.projectId || undefined,
        taskId: selection.level === 'task' ? selection.taskId || undefined : undefined,
      });
      await load();
  ...
  return (
    <section className="grid gap-6">
      <h1 className="text-2xl font-semibold text-[var(--brand)]">Boite mail (inbox)</h1>
      ...
      {assignEmailId ? (() => {
        const email = emails.find((item) => item.id === assignEmailId);
        if (!email) return null;
        const selection = linkSelectionByEmail[email.id] ?? { workspaceId: '', projectId: '', taskId: '', level: 'workspace' };
        ...
        const canSubmit = (() => {
          if (assignStatus === 'submitting') return false;
          if (!selection.workspaceId) return false;
          if (selection.level === 'workspace') return true;
          if (selection.level === 'project') return Boolean(selection.projectId);
          return Boolean(selection.projectId && selection.taskId);
        })();
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/45 p-4">
            <div className="w-[min(760px,96vw)] rounded-xl border border-[var(--line)] bg-white p-5 shadow-2xl">
              ...
              <div className="grid gap-3 lg:grid-cols-3">
                <div className="rounded border border-[var(--line)] bg-[#faf9f6] px-3 py-2 text-sm lg:col-span-3">
                  <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-[#5b5952]">Niveau de validation</p>
                  <div className="flex flex-wrap gap-3">
                    <label className="inline-flex items-center gap-2">
                      <input
                        type="radio"
                        name={`assign-level-${email.id}`}
                        checked={selection.level === 'workspace'}
                        onChange={() => onAssignmentLevelSelection(email.id, 'workspace')}
                        disabled={assignStatus === 'submitting'}
                      />
                      Workspace
                    </label>
                    <label className="inline-flex items-center gap-2">
                      <input
                        type="radio"
                        name={`assign-level-${email.id}`}
                        checked={selection.level === 'project'}
                        onChange={() => onAssignmentLevelSelection(email.id, 'project')}
                        disabled={assignStatus === 'submitting'}
                      />
                      Projet
                    </label>
                    <label className="inline-flex items-center gap-2">
                      <input
                        type="radio"
                        name={`assign-level-${email.id}`}
                        checked={selection.level === 'task'}
                        onChange={() => onAssignmentLevelSelection(email.id, 'task')}
                        disabled={assignStatus === 'submitting'}
                      />
                      Tâche
                    </label>
                ...
                <select
                  ...
                  disabled={!selection.workspaceId || selection.level === 'workspace' || assignStatus === 'submitting'}
                ...
                <select
                  ...
                  disabled={!selection.workspaceId || !selection.projectId || selection.level !== 'task' || assignStatus === 'submitting'}
                  className="rounded border border-[var(--line)] px-3 py-2 text-sm disabled:cursor-not-allowed disabled:opacity-60"
                >
